...
f.order.data<-function(data){
	indx<-order(data$Sampling.Event,data$Camera.Trap.Name,data$td.photo)
	data<-data[indx,]
	data
	}
...
data.frame(data,grp=paste(data$Sampling.Event,".",data$Camera.Trap.Name,".",e.data,sep=""))
...
f.matrix.creator3<-function(data,cams,species){
  require(dplyr)        
  #results object
  res<-list()
...
# function to put data into a form ready to go into jags model
createArrayG <- function(data,species,cams,years, secondPeriods, siteName) {
  # Create a four dimensional matrix with all the values        
  arrayd <- array(data=NA, c(length(species),length(cams),length(years),max(secondPeriods)), dimnames = list(species = species, camera = cams,years = as.character(years), rep = as.character(secondPeriods)))
  
  for (t in 1:length(data)){
    for (s in 1:length(data[[t]]))
      arrayd[s,,t,] <- data[[t]][[s]]
    
  }
  
  ## get the first instance of non-na detection:
  nstart<-sapply(1:dim(arrayd[,,,])[3], function(y) { # This loops across years
    ## This loops across sites, and gets the first non-na slot identity
    sapply(1:dim(arrayd[,,,])[2], function(i) which(!is.na(arrayd[1,i,y,]))[1])
  })
  
  ## get the last instance of non-na detection:
  nend<-sapply(1:dim(arrayd[,,,])[3], function(y) {
    sapply(1:dim(arrayd[,,,])[2], function(i) rev(which(!is.na(arrayd[1,i,y,])))[1])
  })
  
  dimnames(nstart)<-dimnames(nend)<-list(camera=dimnames(arrayd)$camera, year=dimnames(arrayd)$year)
...
make.Z.array<-function(mod, nsite, nspp, nyear, namesArray) {
  
  Zmat<-as.matrix(mod$BUGSoutput$sims.matrix)
  ...
  Zmat<-Zmat[,grep("Z",colnames(Zmat))]
  Zarray<-array(NA, dim=c(my.data$nsp , my.data$nsite , my.data$nyr, dim(Zmat)[1]), dimnames = list(species = namesArray[1]$species, camera = namesArray[2]$camera,years = namesArray[3]$years))
  
  for (sp in 1:my.data$nsp) {
    for (yr in 1:my.data$nyr) {
      for(site in 1: my.data$nsite){
        
        search<-paste("Z\\[",sp,",",site,",",yr,"\\]", sep="" )
        column<-grep(search, colnames(Zmat))
        
        Zarray[sp,site,yr,]<-Zmat[,column]				
      }
    }
  }
