...
Tpecari2017 <- readRDS(here("data", "Tpecari_2017team.rds")) 
y <- Tpecari2017[,2:15]
str(y)
y
SiteCovs <- Tpecari2017[,16:23]
SiteCovs[is.na(SiteCovs)] <- 0 
# check corr in SiteCovs
str(SiteCovs)
cor(SiteCovs)
# elevation, slope, block cor>0.7
...
dataJAGS <- list(y = y, R = nrow(y), T = ncol(y), distWater=distWater, distEdge=distEdge, elevation=elevation)
...
params <- c("alpha.psi", "beta1.psi", "beta2.psi","beta3.psi", "mean.p", "occ.fs", "alpha.p", "z")
